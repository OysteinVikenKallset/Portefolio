...
export default function Fullstack() {
    ...
    const {totalUsers, averageNameLength } = calculateStatistics(users);
    const [leapYearCount, setLeapYearCount] = useState(0);


function calculateLeapYearCount(){
    setLeapYearCount(leapYearCount =0);
    for (let index = 0; index < users.length; index++) {
        const element = users[index];
        if(element.isLeapYearBirthday){
            setLeapYearCount(leapYearCount + 1)
        }
...
useEffect(() => {
    calculateLeapYearCount();
    // calculateLeapYearCount-funksjonen bør være definert i samme komponent eller importert
  }, [users]);
    ...
    function calculateStatistics(users) {
        ...
        return { totalUsers, averageNameLength };
      }
    ...
    return (
        <div>
            ...
            <div>
            <h2>Statistikk</h2>
            <p>Antall brukere: {totalUsers}</p>
            <p>Gjennomsnittlig lengde på navnene: {averageNameLength.toFixed(2)}</p>
            <p>Antall personer født i skuddår: {leapYearCount}</p>
            </div>
        </div>
