...
export default function Fullstack() {
    ...
    const handleUpdateClick = async () => {
        await updateUserData(editingUserId, editedUser);
    ...
    const updateUserData = async (userId, userData) => {
        try {
            const response = await axios.put(`https://your-api-url.com/users/${userId}`, userData);
            return response.data; // Anta at API-et returnerer den oppdaterte brukeren
        } catch (error) {
            if (axios.isAxiosError(error)) {
                console.error('Axios error:', error.response);
                throw new Error('There was an error updating the user data');
            } else {
                        ...
                        {users.map(user => (
                            ...
                            {editingUserId === user.id ? (
                                ...
                                <>
                                    ...
                                    <input name="birthday" type="date" value={editedUser.birthday} onChange={handleChange} />
                                    <button onClick={handleUpdateClick}>Oppdater</button>
                                </>
                            ) : (
